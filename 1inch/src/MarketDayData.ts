...
export function handleMint(event: Deposited): void {
  ...
  let inputTokenDailyInflow: TokenBalance[] = [
    new TokenBalance(pair.token0, pairAddress, prevToken0Inflow.plus(event.params.token0Amount)),
    new TokenBalance(pair.token1, pairAddress, prevToken1Inflow.plus(event.params.token1Amount)),
  ];
...
export function handleBurn(event: Withdrawn): void {
  ...
  let inputTokenDailyOutflow: TokenBalance[] = [
    new TokenBalance(pair.token0, pairAddress, prevToken0Outflow.plus(event.params.token0Amount)),
    new TokenBalance(pair.token1, pairAddress, prevToken1Outflow.plus(event.params.token1Amount)),
  ];
