...
export function handleSync(event: Sync): void {
  ...
  let pair = PairEntity.load(event.address.toHexString()) as PairEntity
  log.info("Sync event - market: {} - transactionHash: {}", [pair.id, transactionHash])

  // IMPORTANT!!!
  // @todo: make sure dstBalance is always token0
  pair.reserve0 = event.params.dstBalance
  pair.reserve1 = event.params.srcBalance
  pair.save()
