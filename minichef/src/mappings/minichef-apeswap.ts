...
export function handleLogPoolAddition(event: LogPoolAddition): void {
  ...
  if (masterChef == null) {
    ...
    let sushi = masterChefContract.BANANA();
    ...
    masterChef.sushiPerSecond = masterChefContract.bananaPerSecond();
    masterChef.save();
  }
  ...
  let protocolName = ProtocolName.APESWAP_FARM;
...
export function handleLogUpdatePool(event: LogUpdatePool): void {
  ...
  // update sushifarm
  sushiFarm.lastRewardTime = event.params.lastRewardTime;
  sushiFarm.totalSupply = event.params.lpSupply;
  sushiFarm.accSushiPerShare = event.params.accBananaPerShare;
  sushiFarm.save();
 ...
 * @param event
 */
export function handleLogSushiPerSecond(event: LogBananaPerSecond): void {
  let masterChef = MasterChef.load(event.address.toHexString());
  masterChef.sushiPerSecond = event.params.bananaPerSecond;
  masterChef.save();
}
